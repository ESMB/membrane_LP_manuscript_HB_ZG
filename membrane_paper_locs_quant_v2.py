...
path = '/Users/user3/Documents/membrane_manuscript_data/reorg_data/'

target_folder = 'fig4_pma1/'
...
for file in membrane_files:
    if filename_contains in file:
        ...
        # m = re.match(r".*Yeast_(?P<condition>....)_(?P<metOH>.*)%metOH_(?P<escin>.*)ul.*posXY(?P<position>.).*", file)
        ...
        for index, row in peakfit_df.iterrows():
            ...
            #print(get_key(loc_coords))

        #peakfit_df.to_csv(path + '/peakfit_membrane_locs/test/' + csv_filename, index=False)
        #print("Written file: " + csv_filename)
